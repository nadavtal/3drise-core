...
export interface PainterlyCloudsPointsMaterialUniforms {
    u_time: {
        value: number;
    };
    u_baseColor: {
...
void main() {
  ...
  float brush = fbm(spherePos * u_brushStrokes + vec3(u_time * 0.05));
  vec3 paintColor = mix(u_baseColor, u_highlightColor, brush);
  
  gl_FragColor = vec4(paintColor * uColor, brush * uOpacity * circle(gl_PointCoord, 0.2));
}
`;
export const PainterlyCloudsPointsMaterial = shaderMaterial({
    u_time: 0,
    u_baseColor: new Vector3(0.7, 0.8, 0.9),
    u_highlightColor: new Vector3(1.0, 1.0, 1.0),
    u_brushStrokes: 4.0,
    uPointSize: 5.0,
...
extend({ PainterlyCloudsPointsMaterial });
