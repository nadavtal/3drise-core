...
export interface CirrusCloudsPointsMaterialUniforms {
    u_time: {
        value: number;
    };
    u_windSpeed: {
...
void main() {
  ...
  float cirrus = fbm(spherePos * 5.0 + vec3(u_time * u_windSpeed, 0.0, 0.0));
  cirrus = pow(cirrus, u_wispy);
  
  gl_FragColor = vec4(u_cloudColor * uColor, cirrus * uOpacity * circle(gl_PointCoord, 0.2));
}
`;
export const CirrusCloudsPointsMaterial = shaderMaterial({
    u_time: 0,
    u_windSpeed: 0.3,
    u_wispy: 2.0,
...
extend({ CirrusCloudsPointsMaterial });
