...
export interface VolumetricCloudsPointsMaterialUniforms {
    u_time: {
        value: number;
    };
    u_cloudDensity: {
...
void main() {
  ...
  float density = fbm(spherePos * 2.0 + vec3(u_time * 0.1)) - u_cloudCoverage;
  density = max(0.0, density) * u_cloudDensity;
  
  vec3 cloudColor = mix(u_shadowColor, u_lightColor, density);
  gl_FragColor = vec4(cloudColor * uColor, density * uOpacity * circle(gl_PointCoord, 0.2));
}
`;
export const VolumetricCloudsPointsMaterial = shaderMaterial({
    u_time: 0,
    u_cloudDensity: 1.5,
    u_cloudCoverage: 0.5,
    u_lightColor: new Vector3(1.0, 1.0, 1.0),
    u_shadowColor: new Vector3(0.3, 0.3, 0.4),
    uPointSize: 5.0,
...
extend({ VolumetricCloudsPointsMaterial });
