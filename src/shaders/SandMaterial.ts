...
export interface SandUniforms {
    u_time: number;
    u_color: THREE.Color;
    u_colorVariation: number;
    u_scale: number;
    u_rippleScale: number;
    u_rippleStrength: number;
    u_rippleAsymmetry: number;
    u_windDir: THREE.Vector2;
    u_grainDensity: number;
    u_roughness: number;
    u_sunDirection: THREE.Vector3;
    u_sunColor: THREE.Color;
    u_sunIntensity: number;
    u_ambient: number;
    u_skyColor: THREE.Color;
    u_groundColor: THREE.Color;
    u_windSpeed: number;
...
export type SandMaterialImpl = THREE.ShaderMaterial & SandUniforms;

declare module '@react-three/fiber' {
    interface ThreeElements {
        sandMaterial: ThreeElements['shaderMaterial'] & Partial<SandUniforms>;
    }
}

const defaults = {
    u_time: 0,
    u_color: new THREE.Color(0.78, 0.66, 0.46),
    u_colorVariation: 0.5,
    u_scale: 1.0,
    u_rippleScale: 1.0,
    u_rippleStrength: 1.0,
    u_rippleAsymmetry: 0.6,
    u_windDir: new THREE.Vector2(1, 0),
    u_grainDensity: 1.0,
    u_roughness: 0.88,
    u_sunDirection: new THREE.Vector3(0.5, 0.82, 0.28),
    u_sunColor: new THREE.Color(1.0, 0.93, 0.82),
    u_sunIntensity: 1.0,
    u_ambient: 1.0,
    u_skyColor: new THREE.Color(0.55, 0.68, 0.92),
    u_groundColor: new THREE.Color(0.45, 0.33, 0.22),
    u_windSpeed: 0.15,
...
`;
export const SandMaterial = shaderMaterial({
    ...defaults,
    u_color: defaults.u_color.clone(),
    u_windDir: defaults.u_windDir.clone(),
    u_sunDirection: defaults.u_sunDirection.clone(),
    u_sunColor: defaults.u_sunColor.clone(),
    u_skyColor: defaults.u_skyColor.clone(),
    u_groundColor: defaults.u_groundColor.clone(),
    u_mouse: defaults.u_mouse.clone(),
}, vertexShader, fragmentShader, (m) => {
    if (!m)
        return;
    // Opaque, lit surface (contrast with Fire's additive glow).
    m.transparent = false;
    m.depthWrite = true;
    m.side = THREE.FrontSide;
    // Required for dFdx/dFdy in the surface-gradient bump on WebGL1/GLSL ES 1.0.
    m.extensions = { ...(m.extensions ?? {}), derivatives: true } as any;
});
extend({ SandMaterial });
