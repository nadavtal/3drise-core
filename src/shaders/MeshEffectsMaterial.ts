...
export interface MeshEffectsMaterialUniforms {
    u_time: {
        value: number;
    };
    u_pointer: {
...
// Vertex shader for mesh geometry
...
`;
// Fragment shader for mesh geometry
...
void main() {
  ...
  if (uHasTexture) {
    // Texture-based rendering using standard UVs
    vec4 colorA = texture2D(uTexture, vUV) * vec4(uColor, 1.0);
    vec4 colorB = texture2D(uTargetTexture, vUV) * vec4(uColor, 1.0);
...
const MeshEffectsMaterial = shaderMaterial({
    u_time: 0,
    uTexture: null,
    uTargetTexture: null,
    uHasTexture: false,
...
extend({ MeshEffectsMaterial });
export { MeshEffectsMaterial };
